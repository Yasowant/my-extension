...
const handlers={},calls=[],reports=[],messages=[];let pick,diagnostics=[],duringPick; 
const start={line:2,compareTo:()=>0};const doc={uri:'file:///example.ts',languageId:'typescript',isClosed:false,version:1};
...
const api={
 ...
 window:{activeTextEditor:editor,showTextDocument:async()=>editor,showQuickPick:async items=>{if(duringPick)duringPick();return pick===undefined?undefined:items[pick];},showInformationMessage:async m=>messages.push(m),showErrorMessage:async m=>messages.push(m)},
...
await handlers['duskForge.troubleshoot']();assert(reports[0].includes('not the final result'));assert(reports[0].includes('Dusk Forge Dark'));
...
calls.length=0;duringPick=()=>doc.version++;await handlers['duskForge.fileProblems']();assert.equal(calls.length,0);assert(messages.some(m=>m.includes('file changed')));
duringPick=()=>{diagnostics=[];};await handlers['duskForge.fileProblems']();assert.equal(calls.length,0);assert(messages.some(m=>m.includes('no longer reported')));duringPick=undefined;
